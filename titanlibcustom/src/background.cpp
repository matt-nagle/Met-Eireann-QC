...
vec titanlibcustom::background(const vec& elevs, const vec& values, int num_min_prof, float min_elev_diff, float value_minp, float value_maxp, BackgroundType background_type, const vec& external_background_values, const ivec& indices_global_outer, bool debug) {

    vec bvalues;
    int p = values.size();

    if( background_type == titanlibcustom::VerticalProfile) {
    ...
    } else if( background_type == titanlibcustom::VerticalProfileTheilSen) {
        // vertical profile is independent from the curr-th observation
        bvalues = ::compute_vertical_profile_Theil_Sen(elevs, elevs, values, num_min_prof, min_elev_diff, debug);
    } else if( background_type == titanlibcustom::MeanOuterCircle){
        // vertical profile is independent from the curr-th observation
        float mean_val = std::accumulate(values.begin(), values.end(), 0.0) / p;
        bvalues.resize(p, mean_val);
    } else if( background_type == titanlibcustom::MedianOuterCircle){
        // vertical profile is independent from the curr-th observation
        float median_val = titanlibcustom::compute_quantile( 0.5, values);
        bvalues.resize(p, median_val);
    } else if( background_type == titanlibcustom::External){
        bvalues = titanlibcustom::subset(external_background_values, indices_global_outer);
    }
    // set the range 
    for(int i=0; i<p; i++) {
...
vec compute_vertical_profile_Theil_Sen(const vec& elevs, const vec& oelevs, const vec& values, int num_min_prof, double min_elev_diff, bool debug) {
    ...
    if ( use_basic) {
        ...
        m_median = titanlibcustom::compute_quantile( 0.5, m);
    }
    for(int i=0; i<n; i++) {
        q[i] = values[i] - m_median * elevs[i];
    }
    double q_median = titanlibcustom::compute_quantile( 0.5, q);
    vec vp(no);
