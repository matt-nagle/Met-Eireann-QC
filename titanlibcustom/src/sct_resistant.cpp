...
ivec titanlibcustom::sct_resistant( const Points& points,
                              const vec& values,
...
/*
 ...
 See also the wiki-pages https://github.com/metno/titanlibcustom/wiki/Spatial-consistency-test
...
*/
    ...
    for(int iteration = 0; iteration < num_iterations; iteration++) {
        ...
        for(int curr=0; curr < p; curr++) {
            ...
            vec eps2_outer   = titanlibcustom::subset(eps2, indices_global_outer);
...
-----------------------------------------------------------------------------*/
    ...
    for(int i=0; i < p_outer; i++) {
        ...
        for(int j=i; j < p_outer; j++) {
            disth(i, j) = titanlibcustom::calc_distance( lats[i], lons[i], lats[j], lons[j]);
        ...
        Dh(i) = titanlibcustom::find_k_closest(Dh_vector, kth_close);
