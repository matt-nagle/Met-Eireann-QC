...
ivec titanlibcustom::duplicate_check(const Points& points, float radius, float vertical_range) {
    ...
    for(int i = 0; i < points.size(); i++) {
        ...
        if(num > 1) {
            ...
            if(titanlibcustom::is_valid(vertical_range) && titanlibcustom::is_valid(elevs[i])) {
                num = 0;
                for(int j = 0; j < indices.size(); j++) {
                    float curr_elev = elevs[indices[j]];
                    if(titanlibcustom::is_valid(curr_elev) && (fabs(elevs[i] - curr_elev) <= vertical_range))
                        checked[indices[j]] = 1;
                }
            }
            else {
