...
bool titanlibcustom::is_valid(float value) {
    return !std::isnan(value) && !std::isinf(value) && value != titanlibcustom::MV;
}
std::string titanlibcustom::version() {
    return __version__;
}

void titanlibcustom::initialize_omp() {
#ifdef _OPENMP
    int num_threads = 1;
    ...
    titanlibcustom::set_omp_threads(num_threads);
#endif
}

void titanlibcustom::set_omp_threads(int num) {
#ifdef _OPENMP
    // omp_set_dynamic(0);
...
bool titanlibcustom::convert_coordinates(const vec& lats, const vec& lons, vec& x_coords, vec& y_coords, vec& z_coords) {
    int N = lats.size();
    x_coords.resize(N);
    y_coords.resize(N);
    z_coords.resize(N);
...
bool titanlibcustom::convert_coordinates(float lat, float lon, float& x_coord, float& y_coord, float& z_coord) {
...
float titanlibcustom::calc_distance(float lat1, float lon1, float lat2, float lon2) {
...
float titanlibcustom::calc_distance(float x0, float y0, float z0, float x1, float y1, float z1) {
    return sqrt((x0 - x1)*(x0 - x1) + (y0 - y1)*(y0 - y1) + (z0 - z1)*(z0 - z1));
}

float titanlibcustom::deg2rad(float deg) {
   return (deg * M_PI / 180);
}

vec titanlibcustom::interpolate_to_points(const vec2& input_lats, const vec2& input_lons, const vec2& input_values, const vec& output_lats, const vec& output_lons) {
    ...
    titanlibcustom::KDTree tree(input_lats_flat, input_lons_flat);
...
float titanlibcustom::compute_quantile(double quantile, const vec& array) {
    int n = array.size();
    if(n == 0) {
...
vec titanlibcustom::subset(const vec& input, const ivec& indices) {
...
Points titanlibcustom::subset(const Points& input, const ivec& indices) {
    int size = indices.size();
...
bool titanlibcustom::invert_matrix(const boost::numeric::ublas::matrix<float>& input, boost::numeric::ublas::matrix<float>& inverse) {
...
bool titanlibcustom::set_indices( const ivec& indices_global_outer_guess, 
                  const ivec& obs_test,
                  const ivec& dqcflags, 
...
float titanlibcustom::find_k_closest(const vec& array, int k) {
    int n = array.size();
    if(n == 0) {
        throw std::runtime_error("Cannot compute quantile on empty array");
    }
