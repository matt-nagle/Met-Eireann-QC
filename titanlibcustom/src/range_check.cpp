...
ivec titanlibcustom::range_check(const vec& values,
        const vec& min,
        const vec& max) {
    ...
    for(int i = 0; i < s; i++) {
        ...
        if(!titanlibcustom::is_valid(values[i]) || values[i] < min[min_i] || values[i] > max[max_i]) {
            flags[i] = 1;
        }
    }
...
ivec titanlibcustom::range_check_climatology(const Points& points,
        const vec& values,
        int unixtime,
        const vec& pos,
        const vec& neg) {
