...
def main():
    parser = argparse.ArgumentParser(description='Runs titanlibcustom benchmarks for processing performance')
    ...
    for i in [1000, N]:
        # points[i] = titanlibcustom.Points(np.linspace(0, 1, i * args.scaling), np.linspace(0, 1, i * args.scaling), np.zeros(i * args.scaling))
        points[i] = titanlibcustom.Points(np.random.rand(i * args.scaling) * args.scaling,
                np.random.rand(i * args.scaling) * args.scaling, np.random.rand(i * args.scaling))
    run = dict()
    radius = 10000
    run[(titanlibcustom.Points, "1e6")] = {"expected": 0.82, "args":(np.linspace(0, 1, int(1e6) * args.scaling), np.linspace(0, 1, int(1e6) * args.scaling), np.zeros(int(1e6) * args.scaling))}
    run[(titanlibcustom.buddy_check, "1e4")] = {"expected": 0.71, "args":(points[N], input[N],
        np.full(N * args.scaling, radius, float), np.ones(N * args.scaling, int) * 10, 0.3, 100.0, 0.0, 1.0, 1)}
    run[(titanlibcustom.isolation_check, "1e4")] = {"expected": 0.71, "args":(points[N], 15, 3000)}
    num_min = 10
    num_max = 50
    inner_radius = 5000
    outer_radius = 50000
    ...
    vertical_scale = 200
    Nsct = 1000
    run[(titanlibcustom.sct, "1000 in 100 km2")] = {"expected": 3.37, "args":(points[Nsct], input[Nsct], num_min, num_max,
    ...
    print("titanlibcustom benchmark (expected results from Intel i7 3.40 Ghz)")
    print("titanlibcustom version %s" % titanlibcustom.version())
    ...
    for key in run.keys()       :
        ...
        if args.function is not None:
            if func.__name__ != args.function:
                continue
        for num_core in num_cores:
            titanlibcustom.set_omp_threads(num_core)
