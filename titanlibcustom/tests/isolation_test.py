...
class IsolationTest(unittest.TestCase):
    ...
    def test_summer_case(self):
        ...
        dz = 100
        s_time = time.time()
        flags = titanlibcustom.isolation_check(points, nmin, radius, dz)
        e_time = time.time()
        print("%.2f %d" %(e_time - s_time, np.sum(flags)))
        s_time = time.time()
        flags = titanlibcustom.isolation_check(points, nmin, radius)
        e_time = time.time()
        print("%.2f %d" %(e_time - s_time, np.sum(flags)))
        print(np.sum(flags))
