...
class BuddyCheckTest(unittest.TestCase):
    def test_1(self):
        print(values)
        flags = titanlibcustom.buddy_check(points, values, radius, num_min, threshold,
    ...
    def test_min_num(self):
        ...
        flags = titanlibcustom.buddy_check(points, values, radius, [20], threshold,
                max_elev_diff, elev_gradient, min_std, num_iterations)
        np.testing.assert_array_equal(flags, [0]*N)
    ...
    def test_elev_gradient(self):
        elevs0 = [0]*9 + [-153.8]
        points0 = titanlibcustom.Points(lats, lons, elevs0)
        flags = titanlibcustom.buddy_check(points0, values, radius, num_min, threshold,
